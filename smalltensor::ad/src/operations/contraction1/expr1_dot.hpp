...
inline val_type operator*(
    ...
    val_type ret(*(Li_(0).get_graph()), 0.) ;
    for (std::size_t n1 = 0; n1 < d1; ++n1){
        ret = ret + Li_(n1) * Ri_(n1);
    }
    return ret;
}
...
inline expr1<val_type,d1,i> operator*(
    ...
    ret_type ret_i;
    for (std::size_t N1 = 0; N1 < d1; ++N1){
      ret_i(N1)= val_type(*(Rj_(0).get_graph()), 0.) ;
    	for (std::size_t n2 = 0; n2 < d2; ++n2)
    	{
	        ret_i(N1) = ret_i(N1) + Lij_(N1,n2) * Rj_(n2);
    	}
    }
    return ret_i;
}
...
inline expr1<val_type,d1,i> operator*(
    ...
    ret_type ret_i;
    for (std::size_t N1 = 0; N1 < d1; ++N1){
      ret_i(N1)=val_type(*(Lj_(0).get_graph()), 0.) ;
    	for (std::size_t n2 = 0; n2 < d2; ++n2)
    	{
