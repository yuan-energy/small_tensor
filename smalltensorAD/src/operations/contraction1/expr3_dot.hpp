...
inline expr3<val_type,d1,d2,d3,i,j,k> operator*(
    ...
    typedef expr3<val_type,d1,d2,d3,i,j,k> ret_type;
    ret_type ret_ijk (*(Rlk_(0,0).get_graph()), 0.); 
    for (std::size_t N1 = 0; N1 < d1; ++N1){
      for (std::size_t N2 = 0; N2 < d2; ++N2){
        for (std::size_t N3 = 0; N3 < d3; ++N3){
          // ret_ijk(N1,N2,N3)=0;
          for (std::size_t n4 = 0; n4 < d4; ++n4){
            ret_ijk(N1,N2,N3) = ret_ijk(N1,N2,N3) + Lijl_(N1,N2,n4) * Rlk_(n4,N3);
          }
        }
      }
    }
...
inline expr3<val_type,d1,d2,d3,i,j,k> operator*(
    ...
    typedef expr3<val_type,d1,d2,d3,i,j,k> ret_type;
    ret_type ret_ijk (*(Lil_(0,0).get_graph()), 0.); 
    for (std::size_t N1 = 0; N1 < d1; ++N1){
      for (std::size_t N2 = 0; N2 < d2; ++N2){
        for (std::size_t N3 = 0; N3 < d3; ++N3){
          // ret_ijk(N1,N2,N3)=0;
          for (std::size_t n4 = 0; n4 < d4; ++n4){
            ret_ijk(N1,N2,N3) = ret_ijk(N1,N2,N3) + Lil_(N1,n4) * Rljk_(n4,N2,N3);
          }
        }
      }
    }
...
inline expr3<val_type,d1,d2,d3,i,j,k> operator*(
    ...
    typedef expr3<val_type,d1,d2,d3,i,j,k> ret_type;
    ret_type ret_ijk (*(Ll_(0).get_graph()), 0.); 
    for (std::size_t N1 = 0; N1 < d1; ++N1){
      for (std::size_t N2 = 0; N2 < d2; ++N2){
        for (std::size_t N3 = 0; N3 < d3; ++N3){
          // ret_ijk(N1,N2,N3)=0;
          for (std::size_t n4 = 0; n4 < d4; ++n4){
            ret_ijk(N1,N2,N3) = ret_ijk(N1,N2,N3) + Ll_(n4) * Rlijk_(n4,N1,N2,N3);
          }
        }
      }
    }
