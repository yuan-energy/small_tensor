...
inline expr2<val_type,d1,d2,i,j> operator* (
    ...
    typedef expr2<val_type,d1,d2,i,j> ret_type;
    ret_type ret_ij( *(Lik_(0,0).get_graph()), 0.);
    for (std::size_t N1 = 0; N1 < d1; ++N1){
      for (std::size_t N2 = 0; N2 < d2; ++N2){
        // ret_ij(N1,N2)=
        for (std::size_t n3 = 0; n3 < d3; ++n3){
          ret_ij(N1,N2) = ret_ij(N1,N2) + Lik_(N1,n3) * Rkj_(n3,N2);
        }
      }
    }
    return ret_ij;
...
inline expr2<val_type,d1,d2,i,j> operator* (
    ...
    typedef expr2<val_type,d1,d2,i,j> ret_type;
    ret_type ret_ij(*(Lk_(0).get_graph()), 0.);
    for (std::size_t N1 = 0; N1 < d1; ++N1){
      for (std::size_t N2 = 0; N2 < d2; ++N2){
        // ret_ij(N1,N2)
        for (std::size_t n3 = 0; n3 < d3; ++n3){
          ret_ij(N1,N2) = ret_ij(N1,N2) + Lk_(n3) * Rkij_(n3,N1,N2);
        }
      }
    }
    return ret_ij;
