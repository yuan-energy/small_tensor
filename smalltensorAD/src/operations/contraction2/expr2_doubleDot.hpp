...
inline expr2<val_type,d1,d2,i,j> operator*(
    ...
    typedef expr2<val_type,d1,d2,i,j> ret_type;
    ret_type ret_ij(*(Likl_(0,0,0).get_graph()), 0.) ;
    for (std::size_t N1 = 0; N1 < d1; ++N1){
      for (std::size_t N2 = 0; N2 < d2; ++N2){
        // ret_ij(N1,N2)=0;
        for (std::size_t n3 = 0; n3 < d3; ++n3){
          for (std::size_t n4 = 0; n4 < d4; ++n4)
          {
            ret_ij(N1,N2) = ret_ij(N1,N2) + Likl_(N1,n3,n4) * Rklj_(n3,n4,N2);
          }
        }
      }
    }
...
inline expr2<val_type,d1,d2,i,j> operator*(
    ...
    typedef expr2<val_type,d1,d2,i,j> ret_type;
    ret_type ret_ij(*(Lkl_(0,0).get_graph()), 0.) ;
    for (std::size_t N1 = 0; N1 < d1; ++N1){
      for (std::size_t N2 = 0; N2 < d2; ++N2){
        // ret_ij(N1,N2)=0;
        for (std::size_t n3 = 0; n3 < d3; ++n3){
          for (std::size_t n4 = 0; n4 < d4; ++n4)
          {
            ret_ij(N1,N2) = ret_ij(N1,N2) + Lkl_(n3,n4) * Rklij_(n3,n4,N1,N2);
          }
        }
      }
    }
