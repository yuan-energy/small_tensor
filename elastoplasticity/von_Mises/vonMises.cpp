...
DTensor2 vonMises::kronecker_delta(3, 3, "identity");

vonMises::vonMises(DVEC constants_prop, double initial_confine)
{
...
int vonMises::setStrainIncr(DTensor2 const& strain_incr){
...
double vonMises::yield_surface(DTensor2 const& dev_stress, double pressure) {
	return sqrt(getJ2(dev_stress)) + _eta * pressure - _cohesion ;
}

double vonMises::yield_surface(DTensor2 const& stress){
...
int vonMises::return2smooth(DTensor2 const& strain_trial, bool& valid){
	...
	{
		cerr<< "ERROR: vonMises::return2smooth iter > RETURN2SMOOTH_MAXITER" <<endl;
		return -1;
	}
...
int vonMises::return2apex(DTensor2 const& strain_trial, double p_trial){
	...
	{
		cerr<< "ERROR: vonMises::return2apex iter > RETURN2APEX_MAXITER" <<endl;
		return -1;
	}
...
void vonMises::CommitState(){
	_commit_stress(_i,_j) = _stress_dev_iter(_i,_j) + _p_iter * kronecker_delta(_i,_j);
	_commit_strain = _strain_iter ; 
	_commit_strain_plastic = _strain_plastic_iter ; 
}

DTensor2 const& vonMises::getCommitStress() const {return _commit_stress;  }
DTensor2 const& vonMises::getCommitStrain() const {return _commit_strain;  }
DTensor2 const& vonMises::getCommitStrainPlastic() const {return _commit_strain_plastic;  }
DTensor4 const& vonMises::getTangentTensor() const {return _Stiffness; }


// Helper
DTensor2 vonMises::getDev(DTensor2 const& strain){
	double vol = strain(_i,_i);
	DTensor2 dev_strain(3,3,0.);
	dev_strain(_i,_j) = strain(_i,_j) - vol/3. * kronecker_delta(_i,_j);
	return dev_strain;
}
double vonMises::getJ2(DTensor2 const& dev_stress){
	return 0.5 * dev_stress(_i,_j) * dev_stress(_i,_j) ;
}

int vonMises::compute_stiffness_return2smooth(double dlambda, DTensor2 const& strain_trial){
...
int vonMises::compute_stiffness_return2apex(){
	cout<<"vonMises::compute_stiffness_return2apex " <<endl;
	_Stiffness = DTensor4(3,3,3,3,0.);
	if(_intersection_factor > 0 ){
		_Stiffness(_i,_j,_k,_l) = _intersection_factor * _Eelastic(_i,_j,_k,_l) + (1.-_intersection_factor) * _Stiffness(_i,_j,_k,_l);
		// first_time = false;
...
double vonMises::backward_zbrentstress(const DTensor2& start_stress,
