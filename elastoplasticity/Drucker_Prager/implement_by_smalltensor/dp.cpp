...
int dp::setStrainIncr(DTensor2 const& strain_incr_){
	_GRAPH.clear() ;
	stresstensor strain_incr(_GRAPH,0.);
	for (int ii = 0; ii < 3; ++ii)	{
		for (int jj = 0; jj < 3; ++jj)		{
			strain_incr(ii,jj).set_value(strain_incr_(ii,jj));
		}
	}
	...
	ad_dual<double> strain_plastic_incr_vol = strain_incr(_i,_i);
...
int dp::return2smooth(stresstensor const& strain_trial, bool& valid){
	...
	ad_dual<double> dlambda = ad_dual<double>(*(_p_iter._graph), 0.);
...
stresstensor dp::getDev(stresstensor const& strain){
	ad_dual<double> vol = (strain(0,0)+strain(1,1)+strain(2,2))/3. ;
...
ad_dual<double> dp::getJ2(stresstensor const& dev_stress){
	// ad_dual<double> ret = 0.5 * dev_stress(_i,_j) * dev_stress(_i,_j) ;
	// 0.5 * dev_stress(_i,_j) * dev_stress(_i,_j) ;
	dev_stress(_i,_j) * dev_stress(_i,_j) ;
	ad_dual<double> ret = ad_dual<double>(_GRAPH,0.) ;
	return ret ;
}
