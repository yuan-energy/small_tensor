...
inline const Expr4 < const Aijkl_times_u < ExprAijkl, typename promotedType, U>,
				typename promotedType, i, j, k, l >
operator* (const ExprAijkl &a, const U & u0)
{
	typedef const Aijkl_times_u < ExprAijkl, typename promotedType, U> ExprObj;
	return Expr4 < ExprObj, typename promotedType, i, j, k, l > (ExprObj (a, u0));
}

// Define
// u0 * A(i,j,k,l) -> Tensor4

template < class A, class T, class U, char i , char j, char k, char l>
inline const Expr4 < const Aijkl_times_u < ExprAijkl, typename promotedType, U>,
				typename promotedType, i, j, k, l >
operator* ( const U & u0, const ExprAijkl &a)
{
	typedef const Aijkl_times_u < ExprAijkl, typename promotedType, U> ExprObj;
	return Expr4 < ExprObj, typename promotedType, i, j, k, l > (ExprObj (a, u0));
}
